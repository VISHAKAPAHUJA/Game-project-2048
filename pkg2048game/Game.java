...
import pkg2048game.Mainn;
import pkg2048game.GameObject;
import pkg2048game.Renderer;
...
public class Game {
	...
	public void render() {
		...
		for(int i = 0 ; i < Mainn.pixels.length; i++) {
			Mainn.pixels[i] = Renderer.pixels[i];
		}
	}
	...
	public void renderText(Graphics2D g) {
		...
		for(int i = 0; i < objects.size(); i++) {
			...
			String s = o.value + "";
			int sw = (int) (g.getFontMetrics().stringWidth(s) / 2 / Mainn.scale);
			g.drawString(s, (int) (o.x + o.width / 2 - sw) * Mainn.scale, (int) (o.y + o.height / 2 + 18) * Mainn.scale);
		}
